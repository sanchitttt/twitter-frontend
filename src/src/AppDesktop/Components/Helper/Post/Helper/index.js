...
export const PollViewer = ({ poll }) => {
  // const {expiresAt,options} = poll;
  const [polls, setPolls] = useState([]);
  const [totalCount, setTotalCount] = useState(0);
  const [timeLeft, setTimeLeft] = useState('');
  // console.log(Math.round(polls[0][1]/parseInt(totalCount)*100))
  console.log();
  useEffect(() => {
    if (poll) {
      // console.log(poll)
          ...
          {polls.map((item, idx) => {
            ...
            >
              <div style={{ width: `${width}`, backgroundColor: '#cfd9de', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div className='individualPollsItem-optionText'>{item[0]}</div>
                <div className='individualPollsItem-percentage'>{ans}%</div>
              </div>
            </div>
          })}
        </div>
...
export const PostMoreTippy = ({ idx }) => {
...
export const MoreOptions = ({ idx }) => {
  ...
  return <div style={{ position: 'relative' }} id={id} >
    ...
    {showTippy && <PostMoreTippy idx={idx} />}
  </div>
